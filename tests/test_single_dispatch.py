...
@pytest.mark.parametrize(['sequences', 'distance'], zip(Cases.SEQUENCES, Results.CDR_DIST))
def test_cdr_dist(sequences, distance):
    result = single_dispatch.cdr_dist(*sequences)
    assert result == distance


@pytest.mark.parametrize(['sequences', 'distance'], zip(Cases.SEQUENCES, Results.LEVENSHTEIN))
def test_levenshtein(sequences, distance):
    result = single_dispatch.levenshtein(*sequences)
    assert result == distance


@pytest.mark.parametrize(['sequences', 'distance'], zip(Cases.EQUAL_SEQUENCE_LENGTH, Results.TCR_DIST))
...
@pytest.mark.parametrize(['sequences', 'distance'], zip(Cases.EQUAL_SEQUENCE_LENGTH, Results.HAMMING))
...
@pytest.mark.parametrize(['sequences', 'distance'], zip(Cases.SEQUENCES, Results.JARO_WINKLER))
